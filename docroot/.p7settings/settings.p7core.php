...
/**
 * Ensure that Acquia's environment variables are set, even for drush.
 *
 * This reaches in to Acquia's own serverside logic and sets the env vars
 * in more or less the same way they do.
 */
...
/**
 *  We need to set CACHE_PREFIX for each AMC instance. Otherwise, every instance
 * will try to write to /tmp, and only the DBTV user has write access to that
 * folder.
 *
 * We're assuming if $_ENV['USER'] is already set, then drush will be able to
 * find an appropriate cache directory for the user. If it's not set, then we
 * need to use AH_SITE_GROUP to get the instance user, and an appropriate cache
 * path for that user.
 */
if (isset($_ENV['AH_SITE_GROUP'])) {
  $cache_prefix = '/mnt/tmp/' . $_ENV['AH_SITE_GROUP'];
  putenv("CACHE_PREFIX=$cache_prefix");
}
 ...
 */
$conf['maintenance_theme'] = 'pub_sauce';
