...
Drupal.behaviors.navbar = {
  ...
  attach: function (context) {
    ...
    $(context).find('#navbar-administration').once('navbar', function () {
      ...
      var isLocked = JSON.parse(localStorage.getItem('Drupal.navbar.trayVerticalLocked'));
      var model = Drupal.navbar.models.navbarModel = new Drupal.navbar.NavbarModel({
        locked: (isLocked != null) ? isLocked : true,
        activeTab: JSON.parse(localStorage.getItem('Drupal.navbar.activeTab')) || 'navbar-item--2'
      });
    ...
    onOrientationToggleClick: function (event) {
      if ($(event.target).hasClass('navbar-toggle')) {
        ...
        if (locked) {
          ...
          localStorage.setItem('Drupal.navbar.trayVerticalLocked', 'false');
        }
        // Update the model.
        this.model.set({
          locked: locked,
