...
(function(ng, modernizr, window) {
  ...
  ng.module('tve.helper', [])
    .factory('helper', [
      '$document', '$sniffer', '$log',
      function($document, $sniffer, $log) {
        var
          TABLET_DESKTOP = 1024,
          MS_IN_SECOND = 1000,
          timezones = {
        ...
        function($http, $q, $cookies, $cookieStore,
          ...
          function openLoginModal(isPreview, options) {
            ...
            if (helper.device.isMobile && (!options || !options.skipMobileBehavior)) {
              return tveModal.showMobileAppModal();
            }
          ...
          var serviceApi = {
            ...
            authWithAdobePass: function(id, redirectUrl) {
              authDefer.promise.then(function() {
                $.cookie(tveCookiesKeys.USERSELECTEDMVPD, id, {
                  expires: 1,
...
(function(ng) {
  ...
  ng.module('tve.auth.directives')
    .directive('tveLoginButton', [
      '$cookies', 'tveAuthConfig', 'authService', 'helper', 'tveModal',
      function($cookies, tveAuthConfig, authService, helper, tveModal) {
        return {
          replace: true,
          restrict: 'EA',
          scope: {},
          template:
            '<a ng-href="{{currentProvider.mvpd_url}}">' +
              '<img ng-if="isAuthenticated && currentProvider && !isRetina" ng-src="{{currentProvider.mvpd_color}}" />' +
              '<img ng-if="isAuthenticated && currentProvider && isRetina" ng-src="{{currentProvider.mvpd_color_2x}}" />' +
              '<i class="inlineLoader" tve-animated-png ng-show="verificationInProgress"></i><span ng-hide="isAuthenticated" ng-bind="text"></span>' +
            '</a>',
            ...
            authService.promise.then(function(status) {
              ...
              if (scope.isAuthenticated = status.isAuthenticated) {
                authService.mvpdService.getMvpd(status.mvpdId).then(function(providerInfo) {
                  ...
                  element
                    .addClass(attr.mvpdLogoClassName ? attr.mvpdLogoClassName : 'providerLogo')
                    .attr('target', '_blank')
                    .find('img').attr({
                      alt: provider.title,
...
(function(ng, tve) {
  ...
  ng.module('tve.auth.directives')
    .directive('tveLogoutButton', [
      'authService',
      function(authService) {
        return {
          replace: true,
          restrict: 'EA',
          template: '<a ng-click="logout();" ng-show="visible" bindonce><span ng-bind="text"></span></a>',
          link: function(scope, element, attr) {
            var SIGN_OUT = Drupal.t('Sign out'),
            ...
            authService.promise.then(function(status) {
              if (status.isAuthenticated) {
                authService.mvpdService.getMvpd(status.mvpdId).then(function(providerInfo) {
                  scope.text = providerInfo.title ?
                    SIGN_OUT_FROM + providerInfo.title :
                    SIGN_OUT;
                ...
                '<div class="menu"><div class="scrollWrap">' +
                  '<ul class="contentList">' +
                    '<li ng-repeat="option in options" ng-mouseover="hightlight($index);" ng-class="{selected: option[valueKey] === selected[valueKey]}" ng-click="select(option);">' +
                      '<a href="" ng-class="{highlight: $index == current}" ng-attr-title="{{option[labelKey]}}" ng-bind-html="option[labelKey]"></a>' +
                    '</li>' +
                  '</ul>' +
                '</div></div>' +
              '</div>',
    ...
    .directive('tvePlayButton', [
      ...
      function(authService, $filter) {
        ...
        return {
          ...
          link: function(scope, element, attr) {
            scope.play = function() {
              authService.playAsset.apply(null, scope.asset ? [scope.asset] : [attr.link, attr.entitlement]);
            };
