...
(function($, window, Drupal) {
  ...
  function AdobePassConnector(config) {
    var NONE_URL = 'none',
        EXPIRED_DATE = -1,
        TRUE_FLAG = '1',
        BASE_PATH = (Drupal && Drupal.settings.basePath) || '/',
        NOOP = function(){},
    ...
    function init() {
      ...
      this.aeMode = this.config.adobePassAccessEnablerMode
                      ? this.config.adobePassAccessEnablerMode
                      : 'flash';

      // Exposing public methods.
      publishApi(this);

      // Embedding AccessEnabled, adding handler.
      // Initialization entry point.
      loadAccessEnabler(this);
    }

    /**
     * Embeds AccessEnabler.
     *
     * @param {Object} settings
     *    Configuration object exposed from adobe_pass.module.
     */
    function loadAccessEnabler(instance) {
      if ('flash' == instance.aeMode) {
        var params = {
              menu: 'false',
              quality: 'low',
        ...
        script_tag.src = config.adobePassAccessEnablerLoc;
        script_tag.id = config.id;
    ...
    function getAccessEnabler() {
      if( window.ae ) {
        // Returns the AccessEnabler object if the AccessEnabler.js has been loaded
        return window.ae;
      }
      else {
        // Returns the AccessEnabler object if the AccessEnabler.swf has been loaded
        return document.getElementById( config.accessEnablerId ); // Note this is the same ID as above
      }
    }
     ...
     * @return A promise wich will be resolved with object with two properties after getting information about selected MVPD:
    ...
    function getSelectedProvider() {
      ...
      if ('flash' == instance.aeMode) {
        d.resolve(accessEnabler.getSelectedProvider());
      } else {
        instance.postponePromise = d;
    ...
    function performPostAuthCheckActions(isAuthenticated) {
      ...
      function runCallback(selectedProvider, nonEntitledMvpd) {
        var status;

        instance.isAuthenticated = nonEntitledMvpd ? false : isAuthenticated;
        instance.currentProvider = nonEntitledMvpd ? null : selectedProvider.MVPD;
      ...
      function trackNonEntitledMvpd(selectedProvider) {
        logout();
        runCallback(selectedProvider, true);
      }

      getSelectedProvider().then(function(selected) {
        if (isAuthenticated) {
          ...
          config.mvpdService.getMvpd(selected.MVPD)
            .then(function(mvpdConfig) {
              ...
              if (status) {
                runCallback(selected, false);
              }
              else {
                trackNonEntitledMvpd(selected);
              }
  ...
  function accessEnablerIsLoaded() {
  ...
  window.swfLoaded = function() {
    accessEnablerIsLoaded();
  };

  /**
   * The entry point for your HTML5/JavaScript interaction.
   * Rather than the swfLoaded() callback function used in the Flash integration.
   */
   ...
   *
   * @param result Object with these properties:
   *                 MVPD     - the currently selected MVPD, or null if no MVPD was selected.
   *                 AE_State - the result of authentication for the current customer, one of "New User",
   *                            "User Not Authenticated", or "User Authenticated
   */
  window.selectedProvider = function(result) {
    // Check if Access Enabler Mode is flash or JavaScript
    if (instance.aeMode != 'flash') {
      tve.adobePass.postponePromise.resolve(result);
    }
  };
