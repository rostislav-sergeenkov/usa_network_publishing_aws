...
//      $anchor.addClass('loaded');
    ...
    initImgShow: function() {
      var lazyloaderItems = $('img.lazyloader-icon');
      $.each(lazyloaderItems, function (index, item) {
        ...
        if(image.attr('data-src')){
          ...
          if (Drupal.behaviors.microsite_scroll.windowView(image)) {
            Drupal.behaviors.microsite_scroll.loadImage(image);
            image.fadeIn('slow');
          }
        }
      });
...
//        Drupal.behaviors.microsite_scroll.micrositeCreateMobileMenu();
      ...
      $('.section').on('scroll', function() {
        Drupal.behaviors.microsite_scroll.initImgShow();
      });
